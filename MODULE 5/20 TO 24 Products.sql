...
-- values are included. Return pro_id, pro_name, pro_price, and pro_com. select * from Products
...
select PRO_NAME as Item_Name, PRO_PRICE as "Price in Rs" from Products;
...
select PRO_NAME,PRO_PRICE from Products order by PRO_PRICE desc ;

select PRO_NAME,PRO_PRICE from Products order by PRO_NAME asc;
...
select avg(PRO_PRICE) as avrageprice ,PRO_COM as comapanycode  from Products group by PRO_COM;
